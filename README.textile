...
All specs pass. Stable

h2. Quick start (Usage)

Define the points on the globe:

The GeoPoint initializer is very flexible with regards to the arguments it can use:
...
Calculate shortest distance in kms between points p1 and p2
...
<pre>
  p1.lat          # latitude
  p2.lon          # longitude
  p3.to_arr       # array representation of [lat, lng]
  p3.reverse_arr! # reverse to_arr to instead return [lng, lat]
  p3.normal_arr!  # return to normal to_arr functionality: [lat, lng]
...
Convert DMS format (degrees-minutes-seconds) to degrees (Float)
